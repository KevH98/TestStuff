...
def get_training_procedures(assigned_roles, employee_name):
    procs = []
    unique_procs = []
    csvfile = mpy_csv.Csv.csv_to_array("/Users/kevinharris/PycharmProjects/EmployeeProject/Book1.csv")
    print(csvfile[0][1])
    for i in range(len(csvfile)):
        for role in assigned_roles:
            if role in csvfile[i][2]:
                procs.append(csvfile[i][0])
...
def add_roles(assigned_roles):
    role = str(input('add roles or "q" to quit adding and get the procuders list: '))
    if (role == 'q' or role == 'Q'):
        print(f"bye the roles are: {assigned_roles}")
